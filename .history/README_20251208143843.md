...
- centraliser les artefacts de preuve (logs, matrices, politiques),
...
- Assurer la traçabilité des actions : logs, versioning, journal d’incidents.
...
- `--dry-run` : validation sans upload
...
## Objectif
Prédire l’heure optimale d’allumage d’un poêle à granulés afin d’atteindre une température cible au moindre coût énergétique.

## Contexte
Je construis un pipeline MLOps minimal viable couvrant ETL, containerisation et exposition d’une API.

## Données
Météo horaire ou quotidienne, type de logement et DPE, historiques d’allumage ou données synthétiques.

## KPI
MAE de l’heure d’allumage prédite en minutes et réduction du temps de chauffe en pourcentage par rapport à une baseline fixe.

## Stack
Python, Pandas, FastAPI, Docker, GitHub Actions ultérieurement, Azure VM et Blob Storage.
...
## Rôles
Solo sur Data, MLOps, API et RGPD.
